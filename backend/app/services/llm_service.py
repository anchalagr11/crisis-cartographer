...
def generate_comparison_prompt(
    crisis_a: str, crisis_b: str, metrics: Dict[str, Any]
) -> str:
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a professional geopolitical analyst.",
                },
...
def generate_mock_insights(
    ...
    if metrics["tag_similarity"] > 0.5:
        similarities.append(
            f"Both {crisis_a} and {crisis_b} share significant structural drivers."
        )
    if metrics["timeline_overlap"] > 0.4:
        similarities.append(
            "These crises occurred within a similar global geopolitical window."
        )

    if metrics["duration_delta_years"] > 5:
        differences.append(
            f"Significant divergence in duration ({metrics['duration_delta_years']} year gap)."
        )
    if metrics["casualty_ratio"] < 0.4:
        differences.append(
            "Major difference in lethality and direct casualty intensity."
        )
    ...
    return {
        "key_similarities": (
            similarities
            if similarities
            else ["No strong structural similarities identified."]
        ),
        "key_differences": (
            differences if differences else ["No major scale divergences identified."]
        ),
        ...
        "data_limitations": [
            "Insights are generated using deterministic fallback rules due to limited data variance."
        ],
    }

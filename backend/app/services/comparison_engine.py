...
from .similarity import calculate_jaccard_similarity, calculate_timeline_overlap, calculate_metric_ratio
...
async def compare_crises(crisis_ids: list) -> ComparisonResult:
    ...
    time_overlap = calculate_timeline_overlap(c1.start_date, c1.end_date, c2.start_date, c2.end_date)
    
    cas_ratio = calculate_metric_ratio(m1["casualties_per_year"], m2["casualties_per_year"])
    disp_ratio = calculate_metric_ratio(m1["displacement_per_year"], m2["displacement_per_year"])
    dur_delta = abs(m1["duration_years"] - m2["duration_years"])
    
    geo_metrics = calculate_geospatial_proximity(c1.regions_affected, c2.regions_affected)
    ...
    return ComparisonResult(
        ...
        aligned_events={
            c1.crisis_id: aligned_a,
            c2.crisis_id: aligned_b
        },
        region_coordinates={

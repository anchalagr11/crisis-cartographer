...
def calculate_relative_month(start_date_str: str, event_date_str: str) -> int:
    ...
    try:
        ...
        start_dt = datetime.strptime(start_date_str, "%Y-%m") if len(start_date_str) == 7 else datetime.strptime(start_date_str, "%Y")
        
    try:
        event_dt = datetime.strptime(event_date_str, "%Y-%m-%d")
    except ValueError:
        event_dt = datetime.strptime(event_date_str, "%Y-%m") if len(event_date_str) == 7 else datetime.strptime(event_date_str, "%Y")
...
def align_timelines(crisis: Crisis) -> List[Dict[str, Any]]:
    ...
    for event in crisis.key_events:
        aligned_events.append({
            "date": event.date,
            "relative_month": calculate_relative_month(crisis.start_date, event.date),
...
def group_events_by_stage(events: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
